        ...
        try:
            if len(np.unique(train_y)) < 2:
                self.logger_object.log(
                    self.file_object,
                    'Only one class present in training data; skipping XGBoost tuning.'
                )
                return None
    ...
    def get_best_model(self,train_x,train_y,test_x,test_y):
        ...
        try:
            ...
            if self.xgboost is not None:
                ...
                self.xgboost_score = float('-inf')
                self.logger_object.log(self.file_object, 'Skipping XGBoost model evaluation for this cluster.')
            ...
            if(self.xgboost_score > self.naive_bayes_score):
